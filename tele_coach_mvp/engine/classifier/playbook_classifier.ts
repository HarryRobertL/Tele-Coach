...
const OBJECTION_ALIAS_PATTERNS: Record<string, WeightedPattern[]> = {
  send_email: [
...
function applyAdaptivePhraseWeight(
  baseWeight: number,
  phrase: string,
  adaptive: AdaptiveWeights
): number {
  const boost = adaptive.phraseBoosts[normalizeAdaptivePhrase(phrase)] ?? 0;
  return Math.max(0.4, baseWeight * (1 + boost));
}

function scoreObjection(
  objectionId: string,
  ...
  const confidence = Math.max(
    0,
    Math.min(1, Number((baselineConfidence + objectionBoost).toFixed(3)))
  );
...
export function detectObjectionId(transcript: string): ObjectionDetection {
  ...
  for (const objection of playbook.objections) {
    const { matched, confidence } = scoreObjection(
      objection.id,
      normalized,
      objection.triggers,
      adaptive
  ...
  if (best.id === "unknown") {
    const genericOrder: Array<{ phrase: string; id: string }> = [
...
export function resetObjectionClassifierState(): void {
  lastResult = {
    id: "unknown",
    matched: [],
    confidence: 0
