...
function loadBridges(): string[] {
  try {
    const filePath = path.join(__dirname, "bridges.json");
    const raw = fs.readFileSync(filePath, "utf-8");
    const data = JSON.parse(raw) as { bridges?: string[] };
 ...
 */
export function pickBridge(seedText: string): string {
